...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Get GitHub token — from connection record or env fallback
  const { data: conn } = await supabase.from("github_connections").select().eq("user_id", user.id).single();
  ...
  const { data: files } = await supabase.from("project_files")
    .select("path, content").eq("project_id", params.id).eq("is_deleted", false);
  ...
  try {
    ...
    // Try to get existing repo first, create if not found
    let repo;
    try {
    ...
    // Get base SHA
    const { data: ref } = await octokit.git.getRef({ owner: ghUser.login, repo: repoName, ref: "heads/main" });
      ...
      files
        ...
        .map(async (f) => {
          const { data: blob } = await octokit.git.createBlob({
            owner: ghUser.login, repo: repoName,
    ...
    const { data: envBlob } = await octokit.git.createBlob({ owner: ghUser.login, repo: repoName, content: Buffer.from(envExample).toString("base64"), encoding: "base64" });
    treeItems.push({ path: ".env.example", mode: "100644", type: "blob", sha: envBlob.sha });

    const { data: tree } = await octokit.git.createTree({ owner: ghUser.login, repo: repoName, base_tree: baseSha, tree: treeItems });
    ...
    await octokit.git.updateRef({ owner: ghUser.login, repo: repoName, ref: "heads/main", sha: commit.sha });

    // Persist
    await admin.from("github_connections").upsert({
      user_id: user.id, login: ghUser.login,
      token_enc: token, scope: ["repo"],
      repo_url: repo.html_url, commit_sha: commit.sha,
    }, { onConflict: "user_id" });

    await admin.from("audit_logs").insert({
      user_id: user.id, project_id: params.id, actor: user.id,
      action: "github.push", resource: "repo", resource_id: repo.html_url,
