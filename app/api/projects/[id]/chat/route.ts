...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!convId) {
    const { data: conv } = await admin.from("conversations").insert({ project_id: params.id, user_id: user.id }).select().single();
    convId = conv?.id;
  }
  ...
  const { data: files } = await supabase.from("project_files").select("path, content").eq("project_id", params.id).limit(15);
...
Be concise and direct. If the project has no files yet, suggest they use the Generate button with a detailed prompt.`;
  ...
  return createSSEStream(async (emit) => {
    const result = await generateText({
      ...
      ctx: {
        projectId: params.id, runId: "chat", userId: user.id, taskId: "chat",
