...
const AGENT_BADGE: Record<string, string> = {
  orchestrator: "badge-violet", frontend: "badge-blue", backend: "badge-blue",
  database: "badge-emerald", qa: "badge-white", repair: "badge-red",
  uiux: "badge-violet", product_manager: "badge-white", architect: "badge-blue",
};
...
export function AgentTimeline({ projectId, stream }: Props) {
  // Pass stream tasks + runningRef so polling suspends during active generation
  const { tasks, loading } = useTaskStatus(projectId, 2000, stream.tasks, stream.runningRef);
    ...
    <div className="flex flex-col h-full">
      <div className="px-4 py-2.5 border-b border-white/[0.05] flex items-center justify-between shrink-0 bg-[#0b0b14]">
        <div className="flex items-center gap-2">
          ...
          {stream.running && (
            <span className="text-[10px] text-violet-400/70 font-medium">· live</span>
          )}
        </div>
        {total > 0 && (
      ...
      {total > 0 && (
        <div className="px-4 pt-3 pb-2 shrink-0">
          ...
          <div className="progress-bar h-1.5">
            ...
            {stream.running && <div className="progress-bar-indeterminate absolute inset-0 rounded-full" />}
          </div>
        </div>
      )}
        ...
        {tasks.map((task, idx) => (
          <div key={task.id} className="task-card" style={{ animationDelay: `${idx * 40}ms` }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              {task.status === "running" && <div className="progress-bar h-0.5 mt-1 w-full progress-bar-indeterminate rounded-full" />}
              {(task.provider || task.model) && (
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="text-[11px] text-white/20 font-mono">{task.provider}/{task.model}</span>
                  {(task.tokens_used ?? 0) > 0 && <span className="text-[11px] px-1.5 py-0.5 rounded-md bg-white/[0.04] border border-white/[0.05] text-white/25 tabular-nums">{(task.tokens_used ?? 0).toLocaleString()} tok</span>}
                  {(task.latency_ms ?? 0) > 0 && <span className="text-[11px] px-1.5 py-0.5 rounded-md bg-white/[0.04] border border-white/[0.05] text-white/25 tabular-nums">{task.latency_ms}ms</span>}
                </div>
              )}
