...
import {
  ...
  Settings, Zap, Download, ChevronLeft, Wrench, AlertTriangle
} from "lucide-react";
...
export function WorkspaceShell({ project: initialProject, initialPanel = "chat" }: { project: Project; initialPanel?: Panel }) {
  ...
  // Called by PromptBar when a generation run starts/ends
    ...
    <div className="h-screen flex flex-col bg-[#0a0a0f] text-white overflow-hidden">
      ...
      {repairError && (
        <div className="flex items-center gap-2 px-4 py-2 bg-red-500/[0.07] border-b border-red-500/15 text-xs text-red-400 shrink-0">
          ...
          {repairError}
          <button onClick={() => setRepairError(null)} className="ml-auto text-red-400/60 hover:text-red-400">✕</button>
        </div>
      )}
