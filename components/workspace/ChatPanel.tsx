...
export function ChatPanel({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0f]">
      {/* Header */}
      <div className="px-4 py-2.5 border-b border-white/[0.05] flex items-center gap-2 shrink-0 bg-[#0b0b14]">
        ...
        <span className="ml-auto text-[10px] text-white/20 tabular-nums">{messages.length - 1} msg{messages.length !== 2 ? "s" : ""}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : "justify-start"} animate-fade-up`}
            ...
            <div className="max-w-[88%] space-y-1">
              <div className={`px-3 py-2 text-sm leading-relaxed ${m.role === "user" ? "bubble-user" : "bubble-assistant"}`}>
        ...
        {loading && (
          <div className="flex gap-2 justify-start animate-fade-in">
            ...
            <div className="bubble-assistant px-3 py-2.5 flex items-center gap-1.5">
              {[0, 1, 2].map(i => (
                <span key={i} className="w-1.5 h-1.5 rounded-full bg-violet-400/50"
                  style={{ animation: `typing-bounce 0.8s ease-in-out infinite`, animationDelay: `${i * 0.15}s` }} />
              ))}
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <div className="p-3 border-t border-white/[0.05] flex gap-2 bg-[#0b0b14]">
        <input
          value={input}
        ...
        <button onClick={send} disabled={loading || !input.trim()}
          className="w-8 h-8 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-30 transition-all flex items-center justify-center shadow-sm shadow-violet-500/20 shrink-0 hover:shadow-violet-500/30">
