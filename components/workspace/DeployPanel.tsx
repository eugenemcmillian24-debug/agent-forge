...
// Graceful: useToast is optional — DeployPanel works without ToastProvider
...
export function DeployPanel({ projectId }: { projectId: string }) {
  ...
  async function handleGitHubPush() {
    ...
    try {
      ...
      if (res.ok) {
        setPushResult(d);
        toast({ variant: "success", title: "Pushed to GitHub", description: d.repoUrl ?? "Repository created successfully" });
      } else {
        setPushResult({ error: d.error });
        toast({ variant: "error", title: "GitHub push failed", description: d.error });
      }
    } catch (err) {
      const msg = String(err);
      setPushResult({ error: msg });
      toast({ variant: "error", title: "GitHub push failed", description: msg });
  ...
  async function handleCloudflareDeploy() {
    ...
    try {
      ...
      if (res.ok) {
        setDeployResult(d);
        toast({ variant: "success", title: "Deployed to Cloudflare", description: d.deployUrl ?? "Deployment complete" });
        const r = await fetch(`/api/projects/${projectId}/deployments`);
        if (r.ok) setDeployments(await r.json());
      } else {
        setDeployResult({ error: d.error });
        toast({ variant: "error", title: "Cloudflare deploy failed", description: d.error });
      }
    } catch (err) {
      const msg = String(err);
      setDeployResult({ error: msg });
      toast({ variant: "error", title: "Cloudflare deploy failed", description: msg });
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] space-y-3.5">
          ...
          {pushResult?.error && (
            <div className="flex items-start gap-2 text-xs text-red-400/90 bg-red-500/[0.06] border border-red-500/15 rounded-lg p-2.5">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>{pushResult.error}</span>
            </div>
          )}
        ...
        <div className="p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] space-y-3.5">
          ...
          {deployResult?.error && (
            <div className="flex items-start gap-2 text-xs text-red-400/90 bg-red-500/[0.06] border border-red-500/15 rounded-lg p-2.5">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" /><span>{deployResult.error}</span>
            </div>
          )}
          <button onClick={handleCloudflareDeploy} disabled={deploying}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-orange-500/10 hover:bg-orange-500/[0.18] border border-orange-500/20 hover:border-orange-500/30 disabled:opacity-50 transition-all text-orange-300 font-medium">
          ...
          <div>
            ...
            <div className="space-y-2">
              {deployments.slice(0, 5).map(d => (
                <div key={d.id} className="flex items-center justify-between p-3.5 rounded-xl border border-white/[0.05] bg-white/[0.02] hover:bg-white/[0.03] transition-colors">
                  <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex items-center gap-3">
                    {d.deploy_url && <a href={d.deploy_url} target="_blank" rel="noopener noreferrer" className="text-xs text-violet-400/80 hover:text-violet-300 transition-colors truncate max-w-[140px]">{d.deploy_url.replace("https://", "")}</a>}
