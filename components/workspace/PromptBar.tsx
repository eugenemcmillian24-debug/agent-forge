...
export function PromptBar({ projectId, stream }: Props) {
  const [prompt,   setPrompt]   = useState("");
  const [profile,  setProfile]  = useState("balanced");
  const [showProf, setShowProf] = useState(false);
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const { startGeneration, running } = stream;
  ...
  return (
    <div className="border-b border-white/[0.05] p-3 bg-[#0b0b14]">
                ...
                <div className="p-1">
                  {PROFILES.map(p => (
                    <button key={p.id} onClick={() => { setProfile(p.id); setShowProf(false); }}
                      className={`w-full text-left px-3 py-2.5 text-xs rounded-lg transition-colors flex items-center justify-between gap-2 ${profile === p.id ? "bg-white/[0.05]" : "hover:bg-white/[0.04]"}`}>
      ...
      {!running && prompt.length === 0 && (
        <p className="text-[10px] text-white/15 mt-2 pl-0.5">Tip: Be specific — include stack, features, and any constraints</p>
      )}
    </div>
  );
}
