...
export function CodeEditor({ projectId, filePath }: { projectId: string; filePath: string | null }) {
  ...
  }, [filePath, projectId, content, saving]);
    ...
    <div className="flex flex-col h-full bg-[#0a0a0f]">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/[0.05] shrink-0 bg-[#0b0b14]">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          {isDirty && (
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0 animate-pulse" title="Unsaved changes" />
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {isDirty && !saved && (
            <span className="text-xs text-amber-400/70 hidden sm:block">Unsaved</span>
          )}
      ...
      {loading ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Line numbers */}
          <div className="w-12 shrink-0 bg-[#0a0a0f] border-r border-white/[0.04] overflow-hidden py-4 select-none"
            aria-hidden="true">
            {content.split("\n").map((_, i) => (
              <div key={i} className="text-right pr-3 text-[11px] font-mono text-white/15 leading-relaxed"
